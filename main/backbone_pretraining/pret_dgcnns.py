...
from cdbs.utils_networks.pretrain_wrappers import DGCNNS_Pretrainer
dataset_folder = os.path.join(data_root, 'ShapeNetCore')
ckpt_name = 'pret_dgcnns'
...
dim_pwf = 192
dim_cdw = 1024
net = DGCNNS_Pretrainer(K, dim_pwf, dim_cdw, dim_vs_cdw).cuda()
max_lr = 1e-3
min_lr = 5e-6
num_epc = 600
...
for epc in range(1, num_epc+1):
    ...
    pretr_loader = DataLoader(pretr_set, batch_size=20, shuffle=True, num_workers=10, worker_init_fn=seed_worker, drop_last=True)
    num_processed = 0
