...
for epc in range(1, num_epc+1):
    ...
    for (pc, cid, name_list) in tqdm(train_loader):
        ...
        optimizer.zero_grad()
        _, ftr_trans_mat, _, _, logits = net(points, cid)
        loss_seg = smooth_cross_entropy(logits.view(bs*num_pts, -1), labels.view(bs*num_pts).long(), eps=0.05)
        loss_reg = ftr_trans_regularizer(ftr_trans_mat)
        loss = loss_seg + loss_reg * 0.0001
        loss.backward()
        optimizer.step()
        num_samples += bs
        epoch_loss_list[0] += (loss.item() * bs)
        epoch_loss_list[1] += (loss_seg.item() * bs)
    ...
    print('epoch: {}, seg loss: {}, reg loss: {}'.format(epc, epoch_loss_list[1], epoch_loss_list[2]))
    cond_1 = (epc<=int(num_epc*0.70) and np.mod(epc, 5)==0)
    cond_2 = (epc>=int(num_epc*0.70) and np.mod(epc, 2)==0)
    if epc<=3 or cond_1 or cond_2:
        ...
        test_miou = np.around(np.array(iou_list).mean()*100, 2)
